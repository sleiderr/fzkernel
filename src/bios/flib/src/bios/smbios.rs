    ...
    fn get_str(&self, num: u8) -> Option<&'static str> {
        ...
        for i in 0..self.data_len {
            ...
            if curr_byte == 0 {
                ...
                if str_count == self.internal.manufacturer {
...
impl SMBIOSEntryTable {
    pub fn get_system_information(&self) -> Option<SMBIOSSystemInfo> {
        let mut curr_mem = self.struct_table_addr;
        let mut sys_info: Option<SMBIOSSystemInfo> = None;
        ...
        while (curr_mem - self.struct_table_addr) < (self.struct_table_len as u32) {
            ...
            match curr_struct_header.struct_type {
                SM_SYSINFO => {
                    let internal: InternalSMBIOSSystemInfo;

                    info!("found SM_SYSINFO at ");
                    hex_print!(curr_mem, u32);

                    unsafe {
                        internal = ptr::read(curr_mem as *mut InternalSMBIOSSystemInfo);
                        let data_addr =
                            curr_mem + mem::size_of::<InternalSMBIOSSystemInfo>() as u32;
